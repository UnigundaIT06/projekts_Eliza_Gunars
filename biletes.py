...
fails = f'bilete_{reisa_numurs}'
def biletes_parbaude():
    try:
        with open(fails, "w", encoding="utf8") as file:
            while True:
                print("Vai šī ir jūsu biļete? (J/N)")
                print("Datums - Galapunkts - Laiks - Reiss - Sēdvieta - Cena")
                bilete = saraksts
                print(n,bilete[n-1]) 
                biletes_parbaude = input("=> ")
                if biletes_parbaude == "J":
                    print(f"Biļete ir saglabāta failā {fails} un aizsūtīta uz jūsu e-pastu! ")
                else:
                    continue
                file.write(bilete )
